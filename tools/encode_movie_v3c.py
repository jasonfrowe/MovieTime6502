#!/usr/bin/env python3
"""
encode_movie_v3c.py — V3C Combined Tile Encoder for MovieTime6502.

V3C keeps the same V3 visual quality but packs both tile layers into one
8192-byte tile block on disk, halving USB bandwidth from 441 KB/s to 249 KB/s.

V3 encodes two layers with complementary nibble sparsity:
...
V3C merges them losslessly:
  combined[i] = (base_tiles[i] & 0x0F) | (overlay_tiles[i] & 0xF0)

The player uses RP6502 opcode 0x2F (read_xram_split) which reads the combined
block from USB and has the Pico split the nibbles into separate XRAM tile slots
at 125 MHz — zero 65C02 cycles spent on the split.

Frame layout on disk (10,656 bytes, MT62 version 2):
...
def encode_frame(frame_bgr: np.ndarray, prev_centers=None) -> tuple[bytes, np.ndarray]:
    ...
    # 6. Binary Packing — combine tile layers into one block
    ...
    tiles1_bytes = build_tileset(list(base_tiles))                            # base    (lo nibble)
    tiles2_bytes = build_tileset(list(overlay_tiles))                         # overlay (hi nibble)
    map2_bytes   = bytes(overlay_map.flatten())
    map1_bytes   = bytes(base_map.flatten())

    # Merge tile layers: lo nibble = base (even cols always zero in overlay),
    # hi nibble = overlay (odd cols always zero in base). Lossless by V3 design.
...
def reconstruct_frame(frame_data: bytes) -> np.ndarray:
    """Decode a V3C frame back to an RGB image (mirrors the Pico-side split)."""
    offset = 0
    pal1_data     = frame_data[offset:offset + 32];   offset += 32
    pal2_data     = frame_data[offset:offset + 32];   offset += 32
    combined_data = frame_data[offset:offset + 8192]; offset += 8192
    map2_data     = frame_data[offset:offset + 1200]; offset += 1200
    map1_data     = frame_data[offset:offset + 1200]

    # Mirror what the Pico's read_xram_split does
...
def cmd_encode(args):
    ...
    print(f"Format: V3C combined ({FRAME_BYTES} bytes/frame, {FRAME_BYTES * args.fps / 1024:.0f} KB/s at {args.fps} fps)")
    print(f"Targeting {args.fps} FPS, ~{total_target_frames} frames total.\n")

    with open(args.output, "wb") as fout:
        fout.write(struct.pack("<4sBBHHHHI", HEADER_MAGIC, HEADER_VERSION, args.fps,
                               SCREEN_W, SCREEN_H, TILE_W, TILE_H, total_target_frames))
...
def cmd_verify(args):
    ...
    with open(args.stream, "rb") as f:
        ...
        if version != HEADER_VERSION:
            print(f"Warning: version {version} != expected {HEADER_VERSION}")
...
def main():
    parser = argparse.ArgumentParser(
        description="MovieTime6502 V3C (Combined Tile) encoder — 249 KB/s at 24 fps"
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    enc = sub.add_parser("encode", help="Encode a video clip into MT62 V3C format")
    ...
    enc.add_argument("--output", default="Movies/MOVIE_metro_v3c.BIN", help="Output binary")
    enc.add_argument("--fps",    type=int, default=24, help="Target FPS")

    ver = sub.add_parser("verify", help="Decode and inspect a V3C packed stream")
    ver.add_argument("stream",       help="MT62 V3C binary stream file")
    ver.add_argument("--debug-dir",  default="debug_verify_v3c", help="Output dir for PNGs")
